...
const getRazorpay = () => createRazorpayInstance();
  ...
  try {
    ...
    if (!process.env.KEY_ID || !process.env.KEY_SECRET) {
      return res.status(500).send({
        success: false,
        message:
          "Razorpay is not configured. Add KEY_ID and KEY_SECRET to backend .env",
      });
    }
    ...
    res.status(isAuthError ? 502 : 500).send({
      success: false,
      message: isAuthError
        ? "Razorpay authentication failed. Verify KEY_ID and KEY_SECRET in backend .env"
        : "Error in creating Razorpay order",
